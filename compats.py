...
texto = Label(janela, text='para hianny nao esquecer os ambientes\n dos compats que ela compila',bg='#8B6508',fg= 'white',font='12')
texto.place(x=70, y=30)

text = Label(janela, text='numero do compilador',bg='#8B7D6B',font='12')
text.place(x=110, y=90)
...
def botaopressionado():
    ...
    amb = amb1.get()
    br = branchs1.get()
    with open(fr"D:\compila homol\compats_ambientes.txt", 'a') as log:
        log.write(f" o compat - {br} -  foi aplicado no ambiente homologacao  - {amb} -  no dia  -{datetime.now().strftime('%Y-%m-%d %H:%M:%S ')}\n")
